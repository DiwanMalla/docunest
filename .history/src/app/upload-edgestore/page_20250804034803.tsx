...
export default function EdgeStoreUploadPage() {
  ...
  const updateFileField = (
    ...
    value: string | number | "pending" | "uploading" | "completed" | "error"
  ) => {
    setFileUploads((prev) =>
      prev.map((upload, i) =>
    ...
    try {
      ...
      const response = await fetch("/api/notes", {
        ...
        body: JSON.stringify({
          ...
          // Don't pass userId - it will be extracted from Clerk auth in the API route
        }),
      });
  ...
  const handleUploadAll = async () => {
    ...
    const filesWithoutTitles = fileUploads.filter(
      (upload) => !upload.title.trim()
    );
    ...
    try {
      ...
      const uploadPromises = fileUploads.map((_, index) =>
        uploadSingleFile(index)
      );
      await Promise.all(uploadPromises);

      // Check if all uploads completed successfully
      const failedUploads = fileUploads.filter(
      ...
      if (failedUploads.length === 0) {
        ...
        setError(
          `${failedUploads.length} file(s) failed to upload. Please check individual file errors.`
        );
      }
    } catch (error) {
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
        <div className="mb-6 sm:mb-8">
          ...
          <p className="mt-2 text-sm sm:text-base text-gray-600">
            Upload multiple PDF or DOCX files to store them securely in the
            cloud. Maximum file size: 100MB per file.
          </p>
        </div>
